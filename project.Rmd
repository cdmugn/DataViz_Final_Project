...
beer_city$StyleColor <- cut(beer_city$Style_rating, 
                        c(3.4, 3.55,3.7,3.85,4.1), include.lowest = T,
                        labels = c('3.4-3.55', '3.55-3.7', '3.7-3.85', '3.85-4.1'), na.rm=TRUE)

StyleCol <- colorFactor(palette = 'BuPu', beer_city$StyleColor)
```
...
Tasks: 
- Find a data set with ratings by beer or brewery, not just style.
...
server <- function(input, output, session) {
  ...
  addTiles() %>% 
  addProviderTiles(providers$Wikimedia) %>% 
  addMarkers(lng = beer_city$lng, lat = beer_city$lat, clusterOptions = markerClusterOptions(singleMarkerMode = TRUE), label = lapply(paste("<strong> Beer name: </strong>", beer_city$Name.x,"<br> <strong> Brewery name: </strong>", beer_city$Name.y), htmltools::HTML))
  })
  ...
  observe({
    style <- style()
    leafletProxy("leafplot2") %>% clearMarkerClusters() %>% addCircleMarkers(lng = style$lng, lat = style$lat, label = lapply(paste( "<strong> Beer name: </strong>", style$Name.x, "<br> <strong> Beer style: </strong>", style$Style, "<br> <strong> Brewery name: </strong>", style$Name.y), htmltools::HTML), clusterOptions = markerClusterOptions(singleMarkerMode = TRUE))
  ...
  observe({
    brewery <- both_breweries()
     leafletProxy("leafplot3") %>% clearMarkers() %>% clearMarkerClusters() %>% fitBounds(min(brewery$lng), min(brewery$lat), max(brewery$lng+1/2), max(brewery$lat-1/2)) %>% addMarkers(lng = brewery$lng, lat = brewery$lat, label = lapply(paste0( " <strong> Brewery name: </strong>", brewery$Brewery), htmltools::HTML), clusterOptions = markerClusterOptions(singleMarkerMode = TRUE))
  })
}
...
- Could color circles by style rating... complicated but possible. - This is very difficult, coming back to it another time.
...
server <- function(input, output, session) {
  ...
  observe({
    style <- style()
    leafletProxy("leafplot2") %>% clearMarkerClusters() %>% addCircleMarkers(lng = style$lng, lat = style$lat, label = lapply(paste( "<strong> Beer name: </strong>", style$Name.x, "<br> <strong> Beer style: </strong>", style$Style, "<br> <strong> Brewery name: </strong>", style$Name.y), htmltools::HTML), color = ~StyleCol(StyleColor), clusterOptions = markerClusterOptions(singleMarkerMode = TRUE)) %>%  addLegend('bottomright', pal = StyleCol, values = beer_city$StyleColor,
            title = 'Style Rating',
            opacity = 1)
