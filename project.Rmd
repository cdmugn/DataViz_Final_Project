...
Tasks:
- make the third map show a marker even if there is only one beer
- make the third map just show the brewery names permanently, not only when you have selected one. 
...
server <- function(input, output, session) {
  ...
  observe({
    brewery <- both_breweries()
     leafletProxy("leafplot3") %>% clearMarkers() %>% fitBounds(min(brewery$lng), min(brewery$lat), max(brewery$lng+1/2), max(brewery$lat-1/2)) %>%   addMarkers(lng = brewery$lng, lat = brewery$lat, popup = paste0( brewery$Brewery), 
    label = paste0( brewery$Brewery))
  })
}
shinyApp(ui, server)
```
- May have to use an if statement so that a marker is only made for each new point and then I can remove clusters and still have markers appear. 
