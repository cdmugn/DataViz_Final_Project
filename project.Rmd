...
url <- "https://github.com/amthomps/BeerRatings/blob/master/breweries.csv"
...
Tasks:
  ...
  - new problem of the zoom map on breweries not showing any markers if there is only one beer at a given brewery
  - maybe find another dataset with more information per brewery to add to the brewery tab.
...
server <- function(input, output, session) {
  ...
  observe({
    brewery <- both_breweries()
     leafletProxy("leafplot3") %>% clearMarkerClusters() %>% fitBounds(min(brewery$lng), min(brewery$lat), max(brewery$lng+1/2), max(brewery$lat-1/2)) %>% addMarkers(lng = brewery$lng, lat = brewery$lat, clusterOptions = markerClusterOptions(), label = paste0( brewery$`Beer name`,"/", brewery$Brewery)) 
  })
}
shinyApp(ui, server)
```
