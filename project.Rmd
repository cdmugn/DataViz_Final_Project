...
- Add a leaflet map that marks on the large scale a dot cluster where each style appears when users picks styles from the style tab.

This has a map but it does not have the observe to change the markers based on which style you choose. Has the observe argument but it does not work yet.
```{r}
library(shiny)
ui <- fluidPage(
  titlePanel("Breweries and Beers in U.S.A."),
...
server <- function(input, output, session) {
  ...
  observe({
    style <- style()
    leafletProxy("leafplot2") %>% clearMarkers() %>% addMarkers(lng = style$lng, lat = style$lat, clusterOptions = markerClusterOptions(), label = content$popup)
  })
