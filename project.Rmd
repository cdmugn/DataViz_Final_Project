...
- add city title to bottom table - this is way more annoying than it should be so I moved on, will figure it out soon.
...
server <- function(input, output, session) {
  ...
  addMarkers(lng = beer_city$lng, lat = beer_city$lat, clusterOptions = markerClusterOptions(), popup = content$popup)
    ...
    observe({
leafletProxy(mapId = "leafplot", data = onecity_df()) %>% clearMarkers() 
  })
...
- this runs so I will change the input of the x so the user can choose either IBU or ABV and they are on the same plot, with the axis label changing confitionally. Next time I work on this. 
