...
- Adding table title with rendertext so it changes with the one city date frame change.
```{r}
library(shiny)
ui <- fluidPage(
...
- Zooming based on the city you choose - problem! I have ideas to fix this but it is taking forever, something to do with leafletProxy and observe
...
server <- function(input, output, session) {
  ...
  addMarkers(lng = beer_city$lng, lat = beer_city$lat, clusterOptions = markerClusterOptions(), popup = content$popup)
    ...
    observe({
leafletProxy("leafplot") %>%
        setView(lng = onecity_df()$lng, lat = onecity_df()$lat, zoom = 10)
  })
