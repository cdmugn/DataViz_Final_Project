...
- Maybe I could color the dots in my lollipop chart by the rating for the style you pick?

- going to do a scale color manual argument where I manually assign breaks and colors that match the scale I built for leaflet. It is not the most efficient method but it will work.
...
server <- function(input, output, session) {
  ...
  output$lollipop1 <- renderPlot({
    ggplot(data = style(), aes(x = Style, y = ABV)) +
  geom_point(data = style2(), aes(y = meanABV), color = "Red", size = 3) + 
  geom_point(aes(color = StyleColor), size = 2) + 
  scale_color_manual(name = "Style Rating",
  ...
  coord_flip() + 
  labs(x = "Beer Style", y = "ABV content", title = "Average ABV content by Beer Style") + theme(title=element_text(size=14,face="bold",color = "darkorange3"), axis.title = element_text(size = 12, face = "plain", color = "black"))
  })
  
  output$lollipop2 <- renderPlot({
    ggplot(data = style(), aes(x = Style, y = IBU)) +
  geom_point(data = style3(), aes(y = meanIBU), color = "Red", size = 3) + 
  geom_point(aes(color = StyleColor), size = 2) + 
  geom_segment((aes(x=Style, xend=Style, y=0, yend=IBU))) + 
  coord_flip() + 
  labs(x = "Beer Style", y = "IBU content", title = "Average IBU content by Beer Style") + theme(title=element_text(size=14,face="bold", color = "darkorange3"), axis.title = element_text(size = 12, face = "plain", color = "black"))
  })
...
Colors in scale: "#F7FCFD" "#E0ECF4" "#BFD3E6" "#9EBCDA" "#8C96C6" "#8C6BB1" "#88419D" "#6E016B"